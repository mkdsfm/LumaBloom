...
`brightness-sensor` is a mixed hardware/software repository with ESP32-C6 and ESP32-C3 firmware tracks plus one Windows desktop application:

- `firmware/firmware_esp32c6/` - `ESP-IDF` firmware for `Waveshare ESP32-C6-LCD-1.47` with `KY-018`
- `firmware/firmware_esp32c3_supermini/` - displayless `ESP-IDF` firmware for `ESP32-C3 Super Mini` with `KY-018`
...
The printable flower assembly is shared by both hardware tracks. The checked-in lower pot and board mount target `ESP32-C6`; the different ESP32-C3 pot has not been modeled or published yet.
...
4. Target-specific README when touching firmware:
   - [`firmware/firmware_esp32c6/README.md`](firmware/firmware_esp32c6/README.md)
   - [`firmware/firmware_esp32c3_supermini/README.md`](firmware/firmware_esp32c3_supermini/README.md)
...
- Both firmware tracks send native ADC readings in `raw` using the same `id="lumabloom"` JSONL contract.
- `pc-app` applies one built-in settings baseline followed by user overrides from `appsettings.json`.
- Firmware-side startup calibration is not required by the current desktop application.
...
### `firmware/firmware_esp32c3_supermini/`

- `ESP-IDF` project targeting `esp32c3`
- Uses `KY-018` on `GPIO4` / `ADC1_CH4`
- Publishes raw telemetry over built-in USB Serial/JTAG and has no display task
...
- `firmware/firmware_esp32c3_supermini/main/*.c` and `main/*.h`
- `firmware/firmware_esp32c6/CMakeLists.txt` and related source-level build files
- `firmware/firmware_esp32c3_supermini/CMakeLists.txt` and related source-level build files
...
Typical commands from `firmware/firmware_esp32c3_supermini/` in an `ESP-IDF` shell:
...
The C3 track has no LCD, local UI, or firmware calibration command.
...
- For ESP32-C3 changes: build target `esp32c3` and call out USB/ADC hardware checks that were not run.
...
- If the task is about the LCD or flower animation: start in `firmware/firmware_esp32c6/`.
- If the task is about the displayless Super Mini: start in `firmware/firmware_esp32c3_supermini/`.
