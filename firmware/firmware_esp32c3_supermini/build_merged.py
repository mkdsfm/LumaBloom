...
VARIANTS = (FirmwareVariant("esp32c3-supermini", "esp32-c3-super-mini"),)
...
def find_idf_path(project_dir: Path) -> Path | None:
    ...
    config_path = project_dir / "build" / "config.env"
...
def find_idf_python(project_dir: Path) -> Path | None:
    ...
    cache = parse_cmake_cache(project_dir / "build" / "CMakeCache.txt")
    ...
    candidates = sorted(Path("C:/Espressif/tools/python").glob("*/venv/Scripts/python.exe"), reverse=True)
    if candidates:
        return candidates[0]
    return None
...
def prepare_environment(project_dir: Path, idf_command: list[str]) -> dict[str, str]:
    ...
    environment["ESP_ROM_ELF_DIR"] = str(idf_path / "components" / "esp_rom" / "esp32c3")
...
def clean_release_artifacts(project_dir: Path, tag: str, dry_run: bool) -> None:
...
def build_variant(project_dir: Path, idf_command: list[str], esptool_command: list[str], environment: dict[str, str], variant: FirmwareVariant, tag: str, skip_build: bool, dry_run: bool) -> tuple[Path, Path]:
    ...
    if not skip_build:
        run(idf_command + ["-B", str(build_dir), "-D", f"PROJECT_VER={tag}", "build"], project_dir, environment, dry_run)
    ...
    run(esptool_command + ["--chip", "esp32c3", "merge-bin", "-o", str(output_path), "-f", "raw", "@flash_args"], build_dir, environment, dry_run)
    manifest = manifest_path(output_path) if dry_run else write_manifest(output_path, variant, tag)
    return output_path, manifest


def main() -> int:
    parser = argparse.ArgumentParser(description="Build a merged binary for the ESP32-C3 Super Mini LumaBloom variant.")
    parser.add_argument("--tag", default="dev", help="Version included in generated filenames")
    parser.add_argument("--variant", action="append", choices=[variant.variant_id for variant in VARIANTS], help="Build only the selected variant")
    parser.add_argument("--skip-build", action="store_true", help="Merge existing build artifacts without rebuilding")
    ...
    clean_release_artifacts(project_dir, tag, args.dry_run)
